...
<%@ page import="com.iamsee.valuebean.*"%>
<%
	String path = request.getContextPath();
	String basePath = request.getScheme() + "://"
			+ request.getServerName() + ":" + request.getServerPort()
			+ path + "/";
	ArrayList postnamelist = (ArrayList) session
			.getAttribute("postnamelist");
	System.out.println(postnamelist.size());
	ArrayList hometownlist = (ArrayList) session
			.getAttribute("hometownlist");
...
<body>
	<form action="servlet/dopageservlet" method="get">
		姓名: <input type="text" style="height: 23px; " id="selectname"
			name="selectname"> 
		家乡: <select id="hometown" name="hometown"
			...
			%>
			<option value="">没有获取到hometownlist</option>
			<%
				} else {
			%>
			<option value="缺省">所有地区</option>
			<%
				for (int i = 0; i < hometownlist.size(); i++) {
						hometown ht = (hometown) hometownlist.get(i);
			%>

			<option value="<%=ht.getName()%>"><%=ht.getName()%>
			</option>
			<%
				}
				}
			%>
		</select> 部门: <select id="condition" name="condition" style="width: 200px;">
			<%
